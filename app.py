...
ARABIC_KEYWORDS = {
    ...
    "مدة الطلاق": "الطلاق في تونس يستغرق minimum 6 أشهر. يبدأ لتقديم طلب الطلاق ثم محاولة الصلح شهرين (الفصل 32)",
    "كيف يطلب الطلاق": "تقديم طلب للمحكمة. المحكمة تحاول الصلح. إذا فشلوا يحكم بالطلاق (الفصل 30-32)",
    "حضانة الأطفال": "الحضانة للأم أولاً ثم ل��أب. الأولاد عند الأم حتى age 15 ثم الأب (الفصل 58)",
    ...
    "تسليم مجرم": "إذا ارتكب أجنبي جريمة في تونس ولم تسلمه دولته، يحاكم في تونس (مبدأ التسليم的国际)",
    "تسليم crime": "الدولة التي فيها الجاني يجب أن تسلمه أو تحاكمه نفسها",
    "ارتكب جريمة": "يجوز التسليم إذا كان هناك إتفاقية بين الدول أو تسلم اختياري",
}
...
def chat():
    ...
    try:
        ...
        # Simple text search
        context = simple_search(user_message, top_k=5)
        
        if not context:
            # Try keyword match
            for keyword, answer in ARABIC_KEYWORDS.items():
                if keyword in user_norm:
                    return jsonify({"response": answer})
